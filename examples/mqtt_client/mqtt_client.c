...
static void ICACHE_FLASH_ATTR beat_task(void *pvParameters)
{
...
static void ICACHE_FLASH_ATTR topic_received(MessageData *md)
{
	int i;
...
static void ICACHE_FLASH_ATTR wifi_task(void *pvParameters)
{
	uint8_t status	= 0;
	uint8_t retries = 30;
...
#if 0
void mqtt_stuff(void *pvParameters)
{
	struct Network network;
	uint8_t mqtt_buf[100];
	uint8_t mqtt_readbuf[100];
	...
	NewNetwork(&network);
	printf("connecting to mqtt host\r\n");
