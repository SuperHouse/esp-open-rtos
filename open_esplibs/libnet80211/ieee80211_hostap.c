...
static void IRAM hostap_timer_func(struct esf_buf *esf_buf) {
    struct sdk_netif_conninfo *conninfo = sdk_g_ic.v.softap_netif_info->conninfo[0];
    int32_t mode = sdk_wifi_get_phy_mode();
    *(uint16_t *)(esf_buf->frame + 22) = (conninfo->_unknown9c - 1) << 4;
    uint8_t *ptr1 = esf_buf->frame + (mode == 1 ? 23 : 27);

    conninfo->_unknown9c += 1;

    uint8_t *ptr2 = ptr1 + sdk_g_ic.s._unknown288;
    ...
    memcpy(ptr2 + 24 + 5, &sdk_g_ic.v._unknown1d2, 1);

    uint32_t v1 = ptr2[24 + 2];
    if (v1 == 0)
        v1 = ptr2[24 + 3];
    ...
    *(uint32_t *)(esf_buf->frame + 24) = v3;
    *(uint32_t *)(esf_buf->frame + 28) = 0;
...
static void hostap_attach_misc() {
    ...
    struct sdk_netif_conninfo *conninfo = netif_info->conninfo[0];
...
static void softap_stop_free() {
    ...
    netif_info->conninfo[0]->_unknown08 = 0;
...
bool sdk_wifi_softap_stop() {
    ...
    if (end >= 2) {
        do {
            struct sdk_netif_conninfo *conninfo = netif_info->conninfo[count];
            if (conninfo) {
                struct sdk_netif_conninfo *conninfo2 = netif_info->_unknown88;
                netif_info->_unknown88 = conninfo;

                sdk_ieee80211_send_mgmt(netif_info, 160, 4);
                sdk_ieee80211_send_mgmt(netif_info, 192, 2);

                netif_info->_unknown88 = conninfo2;

                sdk_cnx_node_leave(netif_info, netif_info->conninfo[count]);
