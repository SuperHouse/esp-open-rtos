...
/* July 25 2017: ported to esp-open-rtos by Sakari Kapanen (flannelhead) */
...
static void captdnsTask(void *pvParameters) {
	...
	server_addr.sin_port = PP_HTONS(53);
...
void captdnsInit(void) {
	xTaskCreate(captdnsTask, (const char *)"captdns_task", 1200, NULL, 2, NULL);
}
